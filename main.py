...
class DatabaseManager:
    ...
    @contextmanager
    def get_cursor(self):        conn = self._get_connection()
        cursor = conn.cursor()
        try:
            yield cursor
            conn.commit()
    ...
    def add_xp(self, user_id, xp_amount):
        with self.get_cursor() as cur:
            ...
            while new_xp >= (new_level * XP_PER_LEVEL):
                new_level += 1
            cur.execute("UPDATE users SET xp = ?, level = ? WHERE user_id = ?", (new_xp, new_level, user_id))
    ...
    def get_user_stats(self, user_id):
        with self.get_cursor() as cur:
            ...
            cur.execute("SELECT COUNT(*) as total, SUM(CASE WHEN is_correct = 1 THEN 1 ELSE 0 END) as correct FROM quizzes WHERE user_id = ?", (user_id,))
            quiz_stats = dict(cur.fetchone() or {})
            user["quiz_stats"] = quiz_stats
            user["accuracy"] = round(quiz_stats["correct"] / quiz_stats["total"] * 100, 1) if quiz_stats["total"] > 0 else 0
            return user
    ...
    def get_pending_homework(self, user_id):
        with self.get_cursor() as cur:
            cur.execute("SELECT * FROM homework WHERE user_id = ? AND is_completed = 0 ORDER BY due_date", (user_id,))
            return [dict(row) for row in cur.fetchall()]
    ...
    def get_active_reminders(self):
        ...
        with self.get_cursor() as cur:
            cur.execute("SELECT * FROM reminders WHERE reminder_time = ? AND is_active = 1", (current_time,))
            return [dict(row) for row in cur.fetchall()]
class AIManager:
...
class QuizBot:
    ...
    async def cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):        user = update.effective_user
...
💬 /feedback - Contact admin"""
    ...
    async def cmd_study(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if not context.args:            await update.message.reply_text("📚 Usage: /study <topic>")
            return
...
📊 Level: {stats.get('level', 1)}💎 XP: {stats.get('xp', 0)}
...
✅ Accuracy: {stats.get('accuracy', 0)}%"""
    ...
    async def _check_reminders(self):
        ...
        for r in reminders:
            try:
                await self.app.bot.send_message(r["user_id"], f"🔔 {r['message']}")
            except:
                pass
