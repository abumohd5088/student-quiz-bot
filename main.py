...
# ============================================================
# 🔐 CONFIGURATION (SECURE - Reads from Railway Variables)
# ============================================================
logging.basicConfig(
...
class Config:
    ...
    @classmethod
    def validate(cls):
        if not cls.TG_BOT_TOKEN or not cls.GROQ_API_KEY:
            logger.error("❌ Missing Environment Variables! Check Railway Settings.")
            sys.exit(1)
...
class DatabaseManager:
    ...
    def get_homework(self, user_id: int) -> List[Dict]:
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM homework WHERE user_id = ? AND is_completed = 0', (user_id,))
        rows = cursor.fetchall()
        conn.close()
        return [dict(row) for row in rows]
...
class AIManager:
    ...
    def explain_concept(self, concept: str, class_level: int) -> str:
        prompt = f"Explain '{concept}' for Class {class_level} simply."
        try:
            response = self.client.chat.completions.create(
                messages=[{"role": "user", "content": prompt}],
                model=self.model,
...
class StudentBot:
    def __init__(self, token: str, db: DatabaseManager, ai: AIManager):
        ...
        self.daily_facts = {
            'science': ["🌍 Earth is the only planet not named after a god.", "💧 Water exists in 3 states naturally."],
            'history': ["🏛️ Great Wall of China is NOT visible from space.", "📜 Shortest war was 38 minutes."],
            'motivation": ["💪 'Expert was once a beginner.'", "🌟 'Success is sum of small efforts.'"]
        }
        ...
        if poll_id in self.active_quizzes:
            ...
            if option_ids:
                ...
                if row:
                    ...
                    msg = "✅ **CORRECT!** +{} XP".format(row['xp_reward']) if is_correct else "❌ **WRONG**"
